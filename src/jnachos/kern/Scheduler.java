...
public class Scheduler {
	...
	private static HashMap<Integer,NachosProcess> processHashMap;
	...
	Scheduler() {
		...
		processHashMap = new HashMap<Integer,NachosProcess>();
	}
	...
	public static boolean containsProcess(int Pid) {
		for (NachosProcess np : readyList) {
			if (np.getpid() == Pid) {
				return true;
			}
	...
	public static void addWaitingProcess(int pid, NachosProcess pWaiting) {
		Debug.print('t', "Putting process " + pWaiting.getName() + " on waiting list.\n");
		processHashMap.put(pid, pWaiting);
	}
	
	public static boolean invokeWaitingProcess(int pid, int exitStatus) {
		if (!processHashMap.containsKey(pid)) {
			return false;
		} else {
			NachosProcess waitingProcess = processHashMap.remove(pid);

			// Set return value to Join()
			waitingProcess.saveUserRegister(2, exitStatus);

			// Put waiting process back to ready queue
